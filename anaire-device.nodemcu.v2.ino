...
// AZ-Delivery RGB LED - https://www.az-delivery.de/es/products/led-rgb-modul?_pos=2&_sid=db0901eae&_ss=r
...
//#define AZDelivery_ESP8266_ESP_12F_NodeMCU_Lua_Amica_V2 2
//int card = AZDelivery_ESP8266_ESP_12F_NodeMCU_Lua_Amica_V2;
int card = 2;
...
//const unsigned long CO2_WARMING_TIME = 6000; // MH-Z14A CO2 sensor warming time: 6s
...
// AZ-Delivery LED KY-016
int status_led_blue_gpio = 4;  // GPIO4 (D2)
int status_led_green_gpio = 0; // GPIO0 (D3) 
int status_led_red_gpio = 2;   // GPIO2 (D4)

// AZ-Delivery Push button: GPIO12 (D6)
int push_button_gpio = 12;

// AZ-Delivery Active Buzzer: GPIO14 (D5)
int buzzer_gpio = 14;

// Status info
int builtin_led_pin = 16; // GPIO16 (D0) builtin LED, used to provide visual insights to developers
...
void setup() {
  ...
  // Status Led
  pinMode(status_led_blue_gpio, OUTPUT);
  pinMode(status_led_green_gpio, OUTPUT);
  pinMode(status_led_red_gpio, OUTPUT);
...
void loop() {
  ...
  // Turn on builtin LED to indicate the start of measurement and evaluation process
  ...
  // Turn off builtin LED to indicate the end of measurement and evaluation process, only if there are not problems
...
void Evaluate_CO2_Value() {
  ...
  if (CO2ppm_value < CO2ppm_warning_threshold) {
    ...
    noTone(buzzer_gpio); // Switch off buzzer
    alarm_ack = false; // Init alarm ack status

    // Status LED: green
    digitalWrite(status_led_red_gpio, LOW); // Switch off red LED
    digitalWrite(status_led_blue_gpio, LOW); // Switch off blue LED
    digitalWrite(status_led_green_gpio, HIGH); // Switch on green LED
  }
  ...
  // Status: warning
    ...
    noTone(buzzer_gpio); // Switch off buzzer
    digitalWrite(status_led_red_gpio, LOW); // Switch off status LED
    alarm_ack = false; // Init alarm ack status

    // Status LED: blue
    digitalWrite(status_led_red_gpio, LOW); // Switch off red LED
    digitalWrite(status_led_blue_gpio, HIGH); // Switch on blue LED
    digitalWrite(status_led_green_gpio, LOW); // Switch off green LED
  }
  ...
  // Status: alarm
    ...
    if (!alarm_ack) {
      ...
      tone(buzzer_gpio, 3636);
      
      // Status LED: red
      digitalWrite(status_led_red_gpio, HIGH); // Switch off red LED
      digitalWrite(status_led_blue_gpio, LOW); // Switch off blue LED
      digitalWrite(status_led_green_gpio, LOW); // Switch on green LED
    }
